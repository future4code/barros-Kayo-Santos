...
describe("Testes signup", () => {
    ...
    test("Teste 3: Erro que deve retornar quando a senha é inválida", async () => {
        expect.assertions(3)
        try {
            await userBusiness.signup("Mockinho","email@email","1234",USER_ROLES.NORMAL)
        } catch (error:any) {
            expect(error).toBeInstanceOf(CustomError)
            expect(error.statusCode).toBe(422) 
    ...
    test("Teste 4: Erro que deve retornar quando quando o tipo de usuário é inválido", async () => {
        expect.assertions(3)
        try {
...
describe.skip("Testes Login", () => {
    ...
    test("Teste 2: Erro que deve retornar quando a senha está errada", async () => {
        expect.assertions(3)
        try {
            await userBusiness.login("email", "1234567")
        } catch (error:any) {
            expect(error).toBeInstanceOf(CustomError)
            expect(error.statusCode).toBe(401) 
            expect(error.message).toBe("Invalid credentials") 
